...
class Parser {
  ...
  parseURL(feedUrl, callback, redirectCount=0) {
    ...
    let prom = axios
      .get(feedUrl, headers, {timeout: this.options.timeout})
      .then(res => {
        if (this.options.maxRedirects && res.status >= 300 && res.staus < 400 && res.headers['location']) {
          if (redirectCount === this.options.maxRedirects) {
            throw new Error("Too many redirects");
          } else {
            const newLocation = url.resolve(feedUrl, res.headers['location']);
            return this.parseURL(newLocation, null, redirectCount + 1);
          }
        }
        ...
        if (encoding == 'windows-1251') {
          return axios
            ...
            .then(res => {
              ...
              let result = conv.convert(res.data).toString();
              return result;
            });
        }
